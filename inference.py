...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [
        (GHOST0, OBS0), (PAC, OBS0),
        (PAC, OBS1), (GHOST1, OBS1)
    ]

    max_distance = X_RANGE + Y_RANGE - 2 + MAX_NOISE
    ...
    variableDomainsDict = {
        ...
        OBS0: list(range(max_distance + 1)),  
        OBS1: list(range(max_distance + 1))   
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        factors = list(allCPTs)
        print("factrs[0] is: ", factors[0])
        print("all CPTS: ", allCPTs)
        for var in eliminationOrder:
            remaining, joinedFactor = joinFactorsByVariable(factors, var)

            if var in joinedFactor.unconditionedVariables() and len(joinedFactor.unconditionedVariables()) > 1:
                joinedFactor = eliminate(joinedFactor, var)
                remaining.append(joinedFactor)

            factors = remaining

        return normalize(joinFactors(remaining))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            #trueDistance = manhattanDistance(pacmanPosition, pos)
            prob_observation_given_pos = self.getObservationProb(observation, pacmanPosition, pos, jailPosition)
            self.beliefs[pos] *= prob_observation_given_pos
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old in self.allPositions:
            ...
            for new in self.allPositions:
                newBeliefs[new] += newPosDist[new] * self.beliefs[old]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if total_weight == 0:
            ...
            for particle, weight in zip(self.particles, weights):
                weight_distribution[particle] += weight
            
            for _ in range(self.numParticles):
                new_particles.append(weight_distribution.sample())
