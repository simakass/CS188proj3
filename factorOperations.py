...
def joinFactors(factors: List[Factor]):
    ...
    print("THESE ARE FACTORS: ", factors)
    variableDomainsDict = factors[0].variableDomainsDict()
    
    resultFactor = Factor(list(unconditionedVariables), list(conditionedVariables), variableDomainsDict)

    for assignmentDict in resultFactor.getAllPossibleAssignmentDicts():
        probabilityProduct = 1.0
        for factor in factors:
            probabilityProduct *= factor.getProbability(assignmentDict)
        resultFactor.setProbability(assignmentDict, probabilityProduct)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        new = Factor(list(unconditioned_vars), list(conditioned_vars), factor.variableDomainsDict())

        for assignmentDict in new.getAllPossibleAssignmentDicts():
            probability = 0.0
            ...
            new.setProbability(assignmentDict, probability)
